...
def run(
    ...
    global last_particle_position
    info_for_naming = '{} Fpk={:.1f} W={:.0f} dt={:.2f} k={:.1f} Fpv={:.1f} Fdm={:.1f}'.format(sled_conditions, pulling_force_k, normal_force, dt, sled_k, pulling_force_v, damp_force_multiplier)
...
for num_sled in [9, 13, 17]:
    for pull_v in [0.1]:
        for W in xrange(-20, 40, 5):
            run(W, (num_sled, 25), pull_v)